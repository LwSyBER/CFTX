...
def train_repeat(X,y,
                   ...
                   verbose = False):
    """
    Trains multiple MLP models with specified hidden layer sizes, repeated several times.

    Parameters:
        X (array-like): Feature matrix.
        y (array-like): Target values.
        hidden_layer_sizes (list of tuples): List of architectures to train (e.g., [(50,), (100, 50)]).
        nb_repeat (int): Number of times to repeat training for each architecture.
        verbose (bool): If True, prints loss after each model is trained.

    Returns:
        list: Trained MLP models from all repetitions.
    """
    models = []
...
def pick_lowest_loss(model_list):
    """
    Selects and returns the model with the lowest training loss from a list of trained models.

    Parameters:
        model_list (list): List of trained models with a `.loss_` attribute.

    Returns:
        best_model: Model with the lowest loss.
...
def check_params(self, params):
    """
    Checks and formats model hyperparameters for use in training or grid search.

    If all values in `params` are not lists, assumes fixed parameters (no search).
    Otherwise, prepares parameter lists for hyperparameter search.

    Parameters:
        params (dict): Dictionary of model hyperparameters.

    Returns:
        self: The instance with updated `model_params` and `params_search` attributes.
    """
...
######################################################################
# Gaussian process class for bayersian opt
...
def predict_rf(model, X):
    """
    Predicts the mean and standard deviation of outputs using a Random Forest model.
    ...
    Returns:
        mean (np.ndarray): Mean prediction across all trees.
        std (np.ndarray): Standard deviation of predictions across all trees.
    """
...
def predict_gp(model, X):
    """
    Predicts the mean and standard deviation using a Gaussian Process model.
    ...
    Returns:
        ...
        std (np.ndarray): Standard deviation of prediction.
    """
...
def predict_ensemble(models, X):
    """
    Predicts the mean and standard deviation using an ensemble of models.
    ...
    Returns:
        pred_mean (np.ndarray): Mean of model predictions.
        pred_std (np.ndarray): Standard deviation across model predictions.
    """
    # Make predictions using each model
    predictions = []
    for model in models:
